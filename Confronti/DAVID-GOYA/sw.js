...
const CACHE=CACHE_PREFIX+'v3-lesson';
...
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(ASSETS.map(path=>new Request(new URL(path,self.location.href),{cache:'reload'})))).then(()=>self.skipWaiting())));
...
self.addEventListener('fetch',event=>{
 ...
 event.respondWith(caches.open(CACHE).then(async cache=>{
  // Refresh the lesson when online; retain its complete cached copy offline.
  if(event.request.mode==='navigate'){
   try{
    const response=await fetch(new Request(event.request,{cache:'no-cache'}));
    if(response.ok){
     if(url.pathname===new URL('./',self.location.href).pathname||url.pathname===new URL('./index.html',self.location.href).pathname){
      await cache.put('./index.html',response.clone());await cache.put('./',response.clone());
     }
     return response;
    }
   }catch{}
   const fallback=await cache.match(event.request)||await cache.match('./index.html');
